...
class LiNA:
    ...
    def fit(self, X, G_sign, scale):
        ...
        # structure of G is known, noises e are ignored
        model_dict = {"F1": ["V1", "V2"],
                      "F2": ["V3", "V4"],
                      "F3": ["V5", "V6"],
                      "F4": ["V7", "V8"],
        ...
        cfa = ConfirmatoryFactorAnalyzer(model_spec, disp=True, max_iter=100000)
        cfa.fit(X, G_sign)
        G_est_nor = cfa.loadings_
        # print(G_est_nor)
...
class MDLiNA:
    ...
    def fit(self, X, G_sign, scale):
        ...
        model_spec = ModelSpecificationParser.parse_model_specification_from_dict(X, model_dict)
        cfa = ConfirmatoryFactorAnalyzer(model_spec, disp=True, max_iter=100000)
        # for better estimation, we input G_sign as well to help initialize the loading matrix
        cfa.fit(X, G_sign)
        G_est = cfa.loadings_
